...
addLayer("e", {
    ...
    effect(){
        ...
        if (hasUpgrade("t", 11)){
            if (eff.gte(Decimal.pow(10,16).mul(upgradeEffect('t', 11)).pow(10,3))) eff = Decimal.pow(10,eff.div(Decimal.pow(10,5)).log10().pow(0.88)).mul(Decimal.pow(10,5))}
...
addLayer("t", {
    ...
    upgrades: {
        11: {
            ...
            effect() {
                return player[this.layer].points.add(1).pow(2).log10().pow(0.85).mul(Decimal.pow(10,3))
            },
            effectDisplay() { return format(upgradeEffect(this.layer, this.id))+"x" }
        },
    },
